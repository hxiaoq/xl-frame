...
export const makeFullUrl = ({ sysCode = '', resPath = '', id = '', resId = '' }) => {
  const token = store.getters.token
  const preUrl = store.getters.sysCodeConfig[sysCode]
  // 有了查询参数就不能用?了
  const sperate = resPath.includes('?') ? '&' : '?'
  return `${deleteEndSperate(preUrl)}${addStartSperate(resPath)}${sperate}id=${id}&token=${token}&resId=${resId}`
}
